  ...
  getOrder = async () => {
    ...
    try {
      const response = await Axios.get(
        'https://mini-project-3a.herokuapp.com/api/pengeluaran',
        {
          headers: {
  ...
  deleteOrder = async (item) => {
    ...
    try {
      const response = await Axios.delete(
        `https://mini-project-3a.herokuapp.com/api/input-pengeluaran/${item.id}`,
        {
          headers: {
            Authorization: `Bearer ${this.props.user.userReducer.user.token}`,
          },
              ...
              <View style={styles.containerKategori}>
                <View style={styles.containerKategori2}>
                  ...
                  <View style={styles.marginLeft40}>
                    <TouchableOpacity
                      onPress={() =>
                        this.props.navigation.navigate('UpdatePengeluaran', {
                          item: item,
                        })
                      }>
