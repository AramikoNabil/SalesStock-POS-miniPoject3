  ...
  getOrder = async () => {
    ...
    try {
      const response = await Axios.get(
        'https://mini-project-3a.herokuapp.com/api/supplier',
        {
          headers: {
  ...
  deleteOrder = async (item) => {
    ...
    try {
      const response = await Axios.delete(
        `https://mini-project-3a.herokuapp.com/api/input-supplier/${item.id}`,
        {
          headers: {
            Authorization: `Bearer ${this.props.user.userReducer.user.token}`,
          },
              ...
              <View style={styles.containerKategori}>
                <View style={styles.containerKategori2}>
                  ...
                  <Text style={styles.fontKategori}>
                    {item.nama} {'\n'}
                    <Text style={styles.fontKategori1} lineBreakMode={'tail'}>
                      {' '}
                      {item.alamat} {'\n'}
                    </Text>
                  </Text>
                  <View style={styles.marginLeft40}>
                    <TouchableOpacity
                      onPress={() =>
                        this.props.navigation.navigate('UpdateSupplier', {
                          item: item,
                        })
                      }>
